...
const indentBlock = (str, depth, tab = '    ') => {
    return str.replace(/^/gm, getTabs(depth, tab));
};
// encloses code in an IIFE
const encloseInIIFE = (code, depth = 1, tab = '    ') => {
    return [
        '(function() {',
...
const includeVersion = (deps, meta, tab = '    ') => {
...
function builder(srcDirectory) {
    ...
    const scriptsStr = (function() {
        ...
        const str = writeScript([
            '\'use strict\';\n',
            `const scripts = [\n${scriptsArrStr}\n];\n`,
            encloseInIIFE(runScriptStr, 1, tab)
        ], 0, tab);
        
        return encloseInIIFE(str, 1, tab);
    }());
    const outputStr = [
        metaStr,
        scriptsStr
